...
class CriticV(tf.keras.Model):
    ...
    def call(self, states):
        ...
        for conv_layer in self.conv_layers:         # 结束后 (1, 256)
            features  =  conv_layer(features)

        for connect_layer in self.connect_layers:   # 结束后 (1, 32)
...
class SAC(OffPolicyAgent):
    def __init__(
        ...
        self._setup_actor(state_shape, action_dim, lr, max_action)      # 策略
        self._setup_critic_v(state_shape, lr)                           # 状态价值
        self._setup_critic_q(state_shape, action_dim, lr)               # 动作价值

        # Set hyper-parameters
        self.tau  =  tau
        self.auto_alpha  =  auto_alpha

        if auto_alpha:                      # 当argo = sac/policy_constraint, auto_alpha = True
        ...
        else:                               # 当argo = value_penalty, auto_alpha = False
            self.alpha  =  alpha
    ...
    def train(self, states, actions, next_states, rewards, dones, weights = None):
        ...
        tf.summary.scalar(name = self.policy_name + "/actor_loss", data = actor_loss)       # name =  SAC 即输入的算法名称        tf.summary.scalar(name = self.policy_name + "/critic_V_loss", data = vf_loss)
    ...
    def _train_body(self, states, actions, next_states, rewards, dones, weights):
        with tf.device(self.device):
            ...
            rewards  =  tf.squeeze(rewards, axis = 1)           # 从张量形状中移除大小为1的维度
            dones  =  tf.squeeze(dones, axis = 1)

            not_dones  =  1. - tf.cast(dones, dtype = tf.float32)   # 数据类型转换
            ...
            with tf.GradientTape(persistent = True) as tape:
                ...
                current_v  =  self.vf(states)           # 计算状态价值

                # Resample actions to update V
                sample_actions, logp, entropy  =  self.actor(states)
                current_q1  =  self.qf1(states, sample_actions)             # 动作价值
