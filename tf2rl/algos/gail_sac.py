...
from tf2rl.policies.mul_gaussian_actor import ExpertGuidedGaussianActor

class CriticV(tf.keras.Model):
    def __init__(self, state_shape, name = 'vf'):
        super().__init__(name = name)

        self.conv_layers  =  [Conv2D(16, 3, strides = 3, activation = 'relu'),
                              Conv2D(64, 3, strides = 2, activation = 'relu'),
                            Conv2D(256, 3, strides = 2, activation = 'relu'),
                            GlobalAveragePooling2D()]
        self.connect_layers  =  [Dense(64, activation = 'relu'), Dense(16, activation = 'relu')]
    ...
    def call(self, states):
        features  =  states                         # (1, 80, 80, 9)

        for conv_layer in self.conv_layers:         # (1, 256)
            features  =  conv_layer(features)

        for connect_layer in self.connect_layers:   #  (1, 32)
            features  =  connect_layer(features)

        values  =  self.out_layer(features)         #  (1, 1)

        return tf.squeeze(values, axis = 1)         #  (1,)
...
class CriticQ(tf.keras.Model):
    def __init__(self, state_shape, action_dim, name = 'qf'):
        ...
        dummy_state  =  tf.constant(np.zeros(shape = (1,) + state_shape, dtype = np.float32))       # (1, 80, 80, 9)
    ...
    def call(self, states, actions):
        ...
        action  =  self.act_layers[0](actions)          # (1, 64)
        features_action  =  tf.concat([features, action], axis = 1)     # (1, 320)

        for connect_layer in self.connect_layers:
            features_action  =  connect_layer(features_action)

        values  =  self.out_layer(features_action)              # (1, 1)
        return tf.squeeze(values, axis = 1)                     #  (1,)

class GAIL_SAC(OffPolicyAgent):
    def __init__(
            self,
            state_shape,
            action_dim,
        ...
        self._uncertainty  =  uncertainty       # f
        ...
        if auto_alpha:                      # argo = sac/policy_constraint, auto_alpha = True
        ...
        else:                               # argo = value_penalty, auto_alpha = False
            self.alpha  =  alpha

        self.state_ndim  =  len(state_shape)                                    # 3
    ...
    def get_action(self, state, test = False):
        ...
        is_single_state  =  len(state.shape)  ==  self.state_ndim               # True
    ...
    def _get_action_body(self, state, test):
        ...
        if test:
            ...
            actions, log_pis, entropy, std, exp_log_pis, kl = self.actor(state, test)
    ...
    def train(self, states, actions, next_states, rewards, dones, weights = None):
        ...
        td_errors, actor_loss, vf_loss, qf_loss, q_value, logp_min, logp_max, logp_mean,\
              entropy_mean  =  self._train_body(
            states, actions, next_states, rewards, dones, weights)

        tf.summary.scalar(name = self.policy_name + "/actor_loss", data = actor_loss)       # name =
    ...
    def _train_body(self, states, actions, next_states, rewards, dones, weights):
        with tf.device(self.device):
            ...
            with tf.GradientTape(persistent = True) as tape:
                ...
                sample_actions, logp, entropy, std, exp_log_pis, kl = self.actor(states)
                ...
                if self.auto_alpha:                     # -2
                    alpha_loss  = -tf.reduce_mean((self.alpha * tf.stop_gradient(kl + self.target_alpha)))
