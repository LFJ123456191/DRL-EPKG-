...
class Trainer:
    def __init__(
        ...
        with open(self._output_dir + '/training_log.csv', 'w', newline = '') as csvfile:
            ...
            writer.writerow(['episode', 'step', 'episodic_reward', 'success_rate', 'episodic_length'])
    ...
    def __call__(self):
        ...
        while total_steps < self._max_steps:                    #
            ...
            if not is_discrete(self._env.action_space):             #
                    env_act = np.clip(action, self._env.action_space.low, self._env.action_space.high)
            else:
                    env_act = action
            ...
            # reward = self._irl.inference(obs, env_act, next_obs)
            replay_buffer.add(obs = obs, act = env_act, next_obs = next_obs, rew = reward, done = done_flag)
            obs  =  next_obs
            ...
            if done or episode_steps  ==  self._episode_max_steps:
                ...
                print()
                print('events ', info['env_obs'].events)
                ...
                self.logger.info("Episode = {0}, T_Steps = {1}, E_Steps = {2}, R = {3: 5.4f}, succ = {4: 5.4f}, FPS = {5:5.2f}".format(
                ...
                if n_episode > 20:
                    ...
                    elif success > best_success or success >= 0.9:
                        best_success  =  success
                        self._policy.actor.network.save('{}/Model/{}_{}_{:.3f}_{}.h5'.format(self._logdir, data_time, n_episode, best_train, success))
            ...
            if total_steps % self._test_interval  ==  0:                            #
                ...
                self.logger.info("Evaluation Total_Steps = {0: 7}, Avg_Reward = {1: 5.4f}, over {2: 2} episodes".format(
    ...
    def evaluate_policy(self, total_steps):
        ...
        for i in range(self._test_episodes):                #
            ...
            for _ in range(self._episode_max_steps):            #
                ...
                next_obs, reward, done, _  =  self._test_env.step({self._test_env.agent_id: env_act})
                next_obs  =  next_obs[self._test_env.agent_id]
                reward  =  reward[self._test_env.agent_id]
                done  =  done[self._test_env.agent_id]

                avg_test_steps +=  1
                if self._save_test_path:                        # value_penalty
                    replay_buffer.add(obs = obs, act = env_act, next_obs = next_obs, rew = reward, done = done)
    ...
    def get_argument(parser = None):
        ...
        parser.add_argument('--use-prioritized-rb', action = 'store_true',
                            help = 'Flag to use prioritized experience replay')         # 标记使用优先体验重放
