...
def get_replay_buffer(policy, env, use_prioritized_rb = False,
    ...
    obs_shape  =  get_space_size(env.observation_space)     # (80, 80, 9)
    kwargs  =  get_default_rb_dict(policy.memory_capacity, env)     # memory_capacity = 2e4

    if size is not None:
        kwargs["size"]  =  size

    # on-policy policy
    if not issubclass(type(policy), OffPolicyAgent):            # 判断前是否是后的子类

        kwargs["size"]  =  policy.horizon                       # gail/ppo时， 512
    ...
    # N-step prioritized
    if use_prioritized_rb and use_nstep_rb:         # sac/value_penalty/policy_constraint时， false,false
    ...
    # prioritized
    if use_prioritized_rb:                      # sac时，false
        return PrioritizedReplayBuffer(**kwargs)

    # N-step
    if use_nstep_rb:                            # sac时，false
        kwargs["Nstep"]  =  {"size": n_step,
                           "gamma": policy.discount,
                           "rew": "rew",
                           "next": "next_obs"}
