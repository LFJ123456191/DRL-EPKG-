...
def load_and_process(exp):
    ...
    for datum in all_files:
        datum['reward']  =  datum['episodic_reward'].ewm(span = 4000).mean()
        datum['success']  =  datum['success_rate'].ewm(span = 4000).mean()
...
axes.set_title(f'{title_name}', fontsize = 16)

if args.metric  ==  'success':
    axes.set_ylim([0, 1])
    axes.set_ylabel('Average success rate', fontsize = 16)
...
plt.legend(loc = 'upper left', labels = labels_1, fontsize = 14)
print('Saving figure...', path_1)
plt.savefig(path_1 + "/draw_results/" + 'training_' + args.scenario + ".jpg")
plt.show()
print('Done!')
